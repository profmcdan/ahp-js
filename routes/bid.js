...
router.post("/:id/add-contractor-price/:contractor_id", (req, res) => {
  ...
  Bid.findById(id)
    .then(bid => {
      ...
      bid.contractors.id(contractor_id).price = price;
      bid
        .save()
        .then(bid => {
          return res.json({
            status: "Success",
            bid: bid.contractors.id(contractor_id),
          });
        })
        .catch(error => {
          console.log(error);
...
router.post("/:id/alternative/upload", (req, res) => {
  ...
  const promises = values.map(document =>
    cloudinary.uploader.upload(document.path),
  );
    ...
    .then(results => {
      ...
      Bid.findById(id)
        .then(bid => {
          ...
          bid.contractors
            .id(contractor_id)
            .documents.unshift({ file_title, filename });
          bid
            .save()
            .then(bid => {
              return res.status(201).json({ bid });
